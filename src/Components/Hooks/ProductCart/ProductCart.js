...
function ProductCart({handleAddtoCart, product}) {
  const {img, category, starCount, stock, price, name} = product;
  const nameShow = text => {
      if(name.length > 135){
        const sliceName = name.slice(0, 125);
        return sliceName + "...";
    }else{
        return text;
    }
  ...
  return (
    <div className='rounded-lg border border-slate-300 bg-slate-100'>
        ...
        <div className='p-4'>
            ...
            <h3 className='mt-2 h-28'>{nameShow(name)}</h3>
